...
TravelOps AX Agent Studio는 공공 관광 데이터를 여행 상품 운영자의 업무 결과물로 바꾸는 AI 워크플로우 시스템입니다. 지역, 기간, 타깃 고객을 입력하면 상품 후보 발굴, 상품 콘셉트 작성, 상세페이지 카피와 FAQ 생성, 운영 리스크 검수, 사람 승인까지 하나의 흐름으로 이어집니다.
...
현재 구현 범위는 Phase 0/1/2/3/4/5/6/7입니다.
...
```bash
cd backend
conda create -y -n travelops-ax-agent-studio python=3.11
conda activate travelops-ax-agent-studio
...
cd frontend
conda activate travelops-ax-agent-studio
conda install -y -n travelops-ax-agent-studio nodejs=20
npm install
npm run dev
```
...
다음 Phase에서는 평가 자동화와 운영 지표를 강화합니다. RAG retrieval recall, faithfulness, tool call accuracy, task success rate, cost per task, latency를 dataset 기반으로 측정하고 Evaluation Dashboard에서 확인할 수 있게 만듭니다.

...
- 상품 개수는 최대 20개로 제한합니다.
- 상품 생성 단계에서는 요청 수를 기준으로 상품 개수를 유지합니다. 직접 연결 가능한 근거가 부족한 상품은 `source_ids`를 빈 배열로 두고 부족 사유를 `needs_review`/`coverage_notes`에 남깁니다.
...
- 출력 규칙: `product_count` 1~20 제한, 명확한 선호/회피 조건만 추출, 상품 생성 제약과 evidence requirement를 한국어 문장으로 작성
...
이 단계는 evidence를 짧게 요약해 버리는 단계가 아닙니다. ProductAgent가 바로 사용할 수 있도록 후보별 근거를 보존하면서, 기존 근거 위에 상품화 해석과 risk guidance를 얹습니다. 원본 TourAPI/RAG/visual/route/signal/theme evidence는 서버 state와 EvidenceFusion 결과에 남고, ResearchSynthesisAgent는 그 원본을 대체하지 않습니다.
...
서버 validation은 Gemini 응답이 후보별 card를 너무 짧게 반환해도 EvidenceFusion base card의 핵심 facts와 `evidence_document_ids`를 보존합니다. Gemini가 `candidate_evidence_cards`를 빈 배열로 반환해도 서버는 기존 EvidenceFusion card를 병합해 유지합니다.
...
- 출력 방식: 원본 evidence card 전체를 다시 쓰지 않고, 후보별 상품화 판단, 스토리 각도, 리스크 보강이 필요한 항목만 `content_id`/`title` 중심으로 짧게 반환
- timeout 대응: 1차 `research_synthesis`는 최대 8,192 output token, timeout 시 `research_synthesis_compact_retry`는 최대 4,096 output token으로 재시도
...
위 예시처럼 Research 출력에는 원본 `usable_facts`와 `evidence_document_ids`를 반복하지 않을 수 있습니다. 해당 값은 EvidenceFusion base card에서 서버가 병합해 보존합니다.
...
- Developer UI의 LLM Calls tab: Gemini 호출과 agent call 기록은 표시
...
- PlannerAgent는 Gemini `purpose=planner`로 호출됩니다.
- ResearchSynthesisAgent는 Gemini `purpose=research_synthesis`로 호출됩니다.

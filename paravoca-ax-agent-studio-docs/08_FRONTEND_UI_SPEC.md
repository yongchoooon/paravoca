...
Run 생성 modal은 상품 개수를 최대 20개로 제한합니다. 사용자가 자연어로 21개 이상을 요청하거나 관광 상품 기획과 무관한 문장을 입력하면 backend `PreflightValidationAgent` 응답을 받아 modal 안에 경고를 표시하고 Run Detail로 넘어가지 않습니다. Workflow Preview에는 이 Preflight gate가 표시되지만, 실제 run이 생성되지 않은 실패이므로 Run Detail의 단계별 진행 목록에는 표시하지 않습니다. 요청 수보다 사용 가능한 근거 데이터가 적으면 ProductAgent가 가능한 개수까지만 생성하고 부족 사유를 상품 card의 확인 항목에 남깁니다.

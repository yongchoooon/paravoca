...
현재 Run Detail의 상세 진행 단계는 개발자가 `agent_steps`와 Gemini 호출 흐름을 확인하기 위한 debug UI입니다. 일반 사용자용 화면에서는 내부 agent 이름과 개별 planner lane을 모두 진행 상태로 보여주지 않고, 더 적은 수의 자연어 단계로 묶어 표시하는 방향을 Phase 10.5 UX 정리 범위에 포함합니다.

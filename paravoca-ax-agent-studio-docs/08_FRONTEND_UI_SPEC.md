...
현재 코드에는 Mantine `AppShell.Header`/`AppShell.Navbar` 기반 전역 navigation shell, Dashboard, message 중심 run 생성 form, run 생성 전 Preflight 검증, run table, task 선택/전체 선택/삭제, React Flow workflow preview, Run Detail, GeoResolver 결과 표시, 지역 후보 안내 UI, 국내 지원 범위 안내, Result Review, approval action, revision history, AI 수정/직접 수정/QA 재검수 modal, Run Detail Evidence의 사용자용 근거 상세/이미지 후보 표시, Data Coverage panel, 추가 데이터 확인 내역 panel, QA Review Avoid 표시가 구현되어 있습니다. Dashboard 화면에는 summary와 Runs table을 함께 유지하고, Workflow Preview는 전역 Navbar에서 독립적으로 접근합니다. Run Detail은 기본 사용자 탭인 `Result Review`, `Evidence + QA`와 개발자용 `Developer` 탭으로 분리되어 있습니다. Poster Studio와 Evaluation Dashboard는 실제 화면으로 전환되었고, Data Sources, Cost Dashboard, Settings 독립 화면은 후속 Phase 목표로 유지합니다.

Run 생성 modal은 상품 개수를 최대 20개로 제한합니다. 사용자가 자연어로 21개 이상을 요청하거나 관광 상품 기획과 무관한 문장을 입력하면 backend `PreflightValidationAgent` 응답을 받아 modal 안에 경고를 표시하고 Run Detail로 넘어가지 않습니다. Workflow Preview에는 이 Preflight gate가 표시되지만, 실제 run이 생성되지 않은 실패이므로 Run Detail의 단계별 진행 목록에는 표시하지 않습니다. 요청 수보다 직접 연결 가능한 근거 데이터가 적어도 ProductAgent는 요청한 상품 수를 유지합니다. 직접 근거가 부족한 상품은 source 연결 없이 생성되고 부족 사유를 상품 card의 확인 항목에 남깁니다.

현재 Run Detail의 상세 진행 단계는 일반 사용자용 요약 단계와 개발자용 상세 단계로 분리되어 있습니다. 일반 사용자용 화면에서는 내부 agent 이름과 개별 planner lane을 모두 진행 상태로 보여주지 않고 `요청 확인`, `지역 해석`, `관광 데이터 확인`, `보강 정보 확인`, `상품 초안 생성`, `검수 및 승인` 단계로 묶어 표시합니다. `agent_steps`, `tool_calls`, `llm_calls`, Raw JSON은 `Developer` 탭에서 확인합니다.
...
- Phase 10.1에서 현재 Dashboard 중심 화면을 `AppShellLayout` 아래로 이동했습니다.
- `AppShell.Navbar`는 단순 장식이 아니라 Dashboard, Workflow Preview, Data Sources, Evaluation, Costs, Poster Studio, Settings로 이동하는 전역 navigation입니다.
- `Runs`는 별도 Navbar item으로 분리하지 않고 Dashboard 안에 기존처럼 summary와 table을 함께 둡니다.
- 아직 구현되지 않은 화면은 future 상태로 보여주고, 실제 기능이 있는 것처럼 표현하지 않습니다.
- Dashboard 내부 navigation성 탭은 제거하고 전역 navbar와 역할이 겹치지 않게 정리했습니다.
...
Run 전체 상품에 실제 연결된 source id의 합집합을 기본 필터로 보여줍니다. Evidence row를 열면 raw API 문자열이 아니라 사용자용 근거 상세를 표시합니다. 요약할 사용자용 설명이 없으면 raw snippet으로 대체하지 않고 요약 칸을 비워둡니다.
...
Evidence detail 기본 화면에는 `content_id`, `source_id`, `source_family`, `region_code`, `detailCommon2`, `detailInfo2`, `trust_level`, `source_confidence`, `retrieved_at` 같은 내부 값이 직접 노출되지 않습니다. 공식 관광 데이터 계열 근거는 `한국관광공사 관광 데이터 기반`으로 한 번만 표시하고, 원본 raw content/metadata와 내부 점수/수집시각은 접힌 Developer raw data 영역에서 확인합니다.
...
- `Comment`와 본문 편집 textarea는 충분한 높이로 보이게 합니다. SNS posts, search keywords, assumptions, not_to_claim처럼 내용이 길어지는 필드는 더 긴 textarea를 사용하되 화면 전체를 과하게 밀어내지 않게 합니다.

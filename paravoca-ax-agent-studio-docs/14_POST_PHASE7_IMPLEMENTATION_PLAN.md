...
이 문서는 Phase 14까지 구현된 PARAVOCA AX Agent Studio를 기준으로, 비용 가시화와 배포/데모 안정화 전에 보강해야 할 구현 단계를 다시 정리한 문서입니다. 기존 `11_IMPLEMENTATION_ROADMAP.md`의 Phase 8/9는 초기 계획 기준이며, 실제 다음 개발은 이 문서의 순서를 우선합니다.
...
- Evaluation dataset/runner, evaluation API, AppShell Evaluation Dashboard
- Poster Studio, poster asset API, Run Detail 상품별 poster 생성, OpenAI Image API 연결
...
- QA issue 설명 품질이 낮아, 어떤 문구가 왜 문제인지 사용자가 바로 알기 어렵다.
- Sales copy, FAQ, SNS, Claims가 여행 상품을 판매하기 위한 마케팅 문구로 충분히 매력적이지 않다.
- Evidence/RAG가 어떤 자료를 왜 근거로 선택했는지 사용자에게 충분히 설명하지 못한다.
- run 실행 때 축적되는 source document와 사전 지식베이스의 역할이 분리되어 있지 않아, 한 번도 실행하지 않은 지역에 대한 기본 근거 품질이 제한된다.
- 이미지 evidence가 상품과 맞지 않는 경우가 있어 관련성 scoring과 UI selection 원리 개선이 필요하다.
- Run Detail과 Evidence/QA 화면에 개발자용 변수명, 내부 상태, provider/tool 중심 문구가 아직 남아 있다.
- AI 수정 후 QA 재검수에서 기존 이슈가 해소되는 대신 새 이슈가 더 많이 생기는 흐름이 있어 revision 품질 기준을 재정의해야 한다.
- Costs 탭에서 LLM/image 생성 비용과 latency를 운영자가 한 화면에서 보는 기능
- 운영 배포를 위한 env/CI/log/cost/monitoring 정리

## 구현 원칙

- 실제 KTO OpenAPI 근거를 우선 사용한다.
- 데이터가 부족하면 고정 데이터로 채우지 않고 공백을 구조화한다.
- 공식 웹 검색/공식 웹 근거 수집은 현재 계획에서 제외한다.
- 공공 API 근거로도 확정할 수 없는 항목은 사용자 입력 또는 내부 DB 확인 대상으로 넘긴다.
...
- 모든 API/tool/LLM 호출은 DB와 로그 파일에서 추적 가능해야 한다.
...
| 웰니스/반려동물/오디오/의료 | `tourism_entities`, `source_documents`, 필요 시 theme-specific metadata | 테마형 상품 근거 |
| 운영자 확인 항목 | `unresolved_gaps`, `needs_review`, `claim_limits` | 집결지, 운영 시간, 예약 조건, 취소 정책처럼 공공 API 근거가 부족한 항목 |
...
- `ThemeDataPlannerAgent`: 웰니스, 반려동물, 오디오, 의료 API 후보를 future/skip 포함해 계획한다.
...
- 반려동물 조건, 웰니스 속성, 오디오 해설 맥락을 상품 구성에 반영
...
- 웰니스/의료관광 데이터는 과장 claim 없이 운영자 확인 항목과 함께 표시된다.
...
- LLM 품질뿐 아니라 지역 해석, 데이터 수집, KTO enrichment, RAG 검색, evidence 기반 상품 생성, QA claim 제한까지 평가한다.

평가 항목:

- geo resolution accuracy
- unsupported/clarification handling
- retrieval result count
- source document indexed count
- expected source family coverage
- enrichment call success rate
- evidence document validity
- product count satisfaction
- product source_id validity
- claim limit compliance
- QA issue detection
- workflow success
- cost/latency
...
- `python -m app.evals.run_eval --dataset smoke --limit 3 --no-live-api` 실행 가능
- live eval에서 `--sleep-between-cases`, `--stop-on-first-failure`로 연속 호출 부하와 실패 조사 흐름을 조절할 수 있다.
- 데이터 보강 source family coverage를 report에서 볼 수 있다.
- 실패 case마다 run_id, 실패 metric, reason을 확인할 수 있다.
- AppShell Evaluation 화면에서 최신 report와 case detail을 볼 수 있다.
- DataGapProfiler는 서버가 만든 gap inventory를 기준으로 Gemini가 ref/group만 선택하고, 서버가 원래 gap 객체와 병합한다.

구현 상태: 완료.

## Phase 14: Poster Studio

목표:

- workflow run에서 생성된 각 여행 상품마다 포스터 초안을 생성하고 저장/다운로드할 수 있게 만든다.

작업:

- poster asset DB model/table 추가
- OpenAI Image API provider 추가
- `.env`의 `OPENAI_API_KEY` 사용
- 영어 deterministic template 기반 prompt builder 구현
- visible poster text와 claim/needs_review constraints 분리
- style preset 3종 구현
- `GET /api/posters`, `GET /api/posters/{poster_id}`, `GET /api/workflow-runs/{run_id}/posters`, `POST /api/workflow-runs/{run_id}/products/{product_id}/posters`, `GET /api/posters/{poster_id}/download`
- Run Detail 상품별 포스터 생성 modal
- Poster Studio image grid와 run/product 기반 생성 flow
- Run Detail과 Poster Studio 양쪽에서 상품 관련 이미지 후보를 보고 최대 3개까지 참조 이미지로 선택하는 flow
- 참조 이미지 후보는 직접 URL 입력이 아니라 evidence 기반 후보만 사용하고, 상품 직접 근거 -> 같은 content_id 후보 순서로 정렬
- 참조 이미지 후보 card에는 API 태그나 URL 대신 이미지명과 thumbnail을 표시하고, 화면별 layout에 맞는 grid로 선택 제공
- 포함할 내용 preview는 말줄임표 대신 2줄 높이로 접고, 화살표 버튼으로 펼침/접기 제공
- Poster Studio의 run/product 선택은 텍스트 편집형 dropdown 대신 Dashboard table과 유사한 compact row selector로 상단 1행 2열에 제공
- Poster Studio 생성 옵션은 상단 run/product 선택 아래에서 좌측 설정 컬럼에 포함할 내용, 스타일/생성, 참조 이미지를 세로 흐름으로 배치
- Poster Studio 참조 이미지는 2열, Run Detail 생성 modal 참조 이미지는 3열 grid로 표시
- Dashboard run table에는 포스터가 있는 run에만 poster count 표시
- 포스터/참조 이미지 preview modal은 기본 상태에서 전체 이미지가 화면 안에 보이고, 별도 확대 버튼 없이 이미지 클릭으로 확대/축소 toggle 처리
- preview modal 확대 비율은 약 30% 수준으로 제한해 기본 확인 상태에서 스크롤바가 생기지 않도록 처리
- nested modal에서 `Esc`는 가장 위의 창만 닫도록 처리
- poster style과 저장 card metadata는 사용자 화면에서 한국어 label로 표시하고, status badge는 failed 상태에만 표시
- Poster Studio poster card에서 연결된 Run Detail을 바로 열 수 있게 처리
- 참조 이미지가 없는 poster prompt는 광역 지명만 보고 유명 랜드마크를 임의 선택하지 않도록 하고, 상품 제목/일정/마케팅 전략/실제 근거 제목·요약에서 더 구체적인 장소, 구, 행사, 문화, 장소 유형을 우선 추론하도록 지시
- background poster generation, 생성 중 loading/skeleton, 실패 error state, preview/download
- 상품 1개당 최대 3개 poster draft 저장 제한과 delete flow
- 생성된 poster image 클릭 시 확대 preview
- API key 없음/provider 실패/storage 실패 시 fake poster 없이 failed 상태와 에러 표시
- OpenAI Image API usage 기반 poster 비용 계산과 `poster_usage` file log 기록
- 생성에 사용한 poster prompt를 `poster_prompts/<run_id>/<poster_id>.json|.md` 파일로 기록

완료 기준:

- Poster Studio 탭에서 run -> product -> 포함 내용 -> style 선택 후 포스터를 생성할 수 있다.
- Run Detail에서 상품별로 run/product 선택 없이 바로 포스터를 생성할 수 있다.
- 두 화면 모두 선택한 상품과 연결된 이미지 후보를 사람이 식별 가능한 이름과 큰 이미지로 확인하고, 이미지를 클릭해 확대 preview를 볼 수 있다.
- 이미지 후보는 상품 `source_ids`와 직접 연결된 evidence를 우선하고, 같은 `content_id`로 묶이는 후보를 후순위로 보여준다.
- 생성 modal을 닫거나 화면을 이동해도 running poster가 background에서 완료되고, 이후 Run Detail과 Poster Studio에서 확인된다.
- 생성된 포스터는 이미지 preview와 download를 제공한다.
- 상품 1개당 최대 3개까지 poster draft를 저장하고, 삭제하면 다시 생성할 수 있다.
- prompt와 provider metadata는 기본 사용자 화면에 노출하지 않고 파일/DB 기록으로 남긴다. 실패한 poster는 사용자 화면에 failed 상태와 에러 요약을 표시한다.
- poster 비용은 `poster_assets.cost_usd`, `provider_response_summary.cost_breakdown`, `backend/logs/poster_usage.*`에서 확인할 수 있다.
- poster prompt는 DB row와 `backend/logs/poster_prompts` 파일 로그에 남고, 사용자 기본 화면에는 노출하지 않는다.
- claim 제한, not_to_claim, needs_review는 visible copy가 아니라 prompt constraints로 들어간다.

구현 상태: 완료.

## Phase 15: Quality Audit and Product Quality Baseline

상세 계획: [`26_PHASE_15_QUALITY_HARDENING_PLAN.md`](./26_PHASE_15_QUALITY_HARDENING_PLAN.md)

목표:

- QA, Marketing, Evidence, RAG, UI copy 품질 문제를 구현 전에 분해하고, eval/run 기반으로 개선 기준을 확정한다.

작업:

- 최근 일반 run과 evaluation dataset 결과를 샘플링해 실패 유형을 분류한다.
- QA issue가 generic message로 나오는 케이스, false positive, 누락 issue를 분류한다.
- AI 수정 후 QA 재검수에서 기존 이슈 해소/신규 이슈 증가가 어떻게 발생하는지 추적한다.
- Sales copy, FAQ, SNS, Claims의 상품성 문제를 예시 기반으로 정리한다.
- RAG source document가 언제 생성/저장/index되는지, run 기반 축적과 사전 ingestion의 차이를 문서화한다.
- Evidence selection과 이미지 후보 selection의 현재 scoring/우선순위 기준을 정리한다.
- 사용자 화면에서 제거해야 할 개발자용 문구, 변수명, 내부 상태명을 목록화한다.

완료 기준:

- 품질 문제를 QA, Marketing, RAG/Evidence, Visual evidence, UI copy, Revision workflow로 나눈 audit 문서가 남는다.
- “사용자에게 보여야 하는 정보 / 숨겨야 하는 정보” 기준이 정리된다.
- Phase 16~20의 구현 체크리스트와 우선순위가 확정된다.

## Phase 16: QA Quality Hardening

목표:

- QA가 실제 문제 문구, 위치, 이유, 수정 방향을 명확히 말하고, 재검수 결과가 예측 가능하게 줄어들도록 만든다.

작업:

- QA prompt와 schema를 재정리한다.
- `issue.message`에는 문제 위치와 실제 문제 문구 인용을 강제한다.
- severity/type 기준을 정리한다.
- 안전한 불확실성 표현을 issue로 잡지 않도록 false positive를 줄인다.
- AI 수정 후 재검수에서는 기존 이슈의 resolved/still_open/new_issue 상태를 비교할 수 있게 한다.
- QA-only revision과 AI partial rewrite revision의 차이를 UI에서 명확히 보여준다.

완료 기준:

- QA issue만 보고도 사용자가 어느 문구를 고쳐야 하는지 알 수 있다.
- AI 수정 후 QA 재검수에서 기존 이슈 해소 여부와 새 이슈가 분리되어 표시된다.
- evaluation regression/quality dataset에서 claim risk 관련 실패가 줄어든다.

## Phase 17: Marketing Output Hardening

목표:

- Sales copy, FAQ, SNS, Claims를 근거를 지키면서도 실제 여행 상품 판매에 도움이 되는 문구로 개선한다.

작업:

- Product -> Marketing prompt를 전면 개선한다.
- Sales copy 구조를 상품 매력, 대상 고객, 경험 가치, 운영 확인 항목이 분리되도록 조정한다.
- FAQ를 운영 리스크 안내만이 아니라 구매 전환에 도움이 되는 질문/답변으로 개선한다.
- SNS 문구의 톤, 길이, 상품별 차별성을 개선한다.
- Claims를 “사용 가능한 주장 / 확인 필요 / 금지 claim”으로 명확히 분리한다.
- 외국인 대상이지만 한국어 운영 문구를 유지하는 현재 정책이 적절한지 재검토한다.

완료 기준:

- Marketing quality judge와 수동 검토에서 문구가 지나치게 딱딱하거나 generic하다는 문제가 줄어든다.
- Claims가 근거와 충돌하지 않으면서도 사용 가능한 셀링 포인트를 제공한다.

## Phase 18: RAG and Evidence Pipeline Hardening

목표:

- RAG가 어떤 자료를 기반으로 동작하는지 명확히 하고, run 기반 축적만으로 생기는 공백을 줄인다.

작업:

- source document 생성, DB 저장, Chroma index, search 흐름을 코드와 DB 기준으로 추적한다.
- run 실행 때 쌓이는 문서와 사전 ingestion 문서의 역할을 분리한다.
- 지역별 기본 seed ingestion 또는 catalog pre-index 전략을 검토한다.
- 한 번도 실행하지 않은 지역에서도 최소한의 기본 근거를 확보하는 방식을 설계한다.
- RAG retrieval relevance, stale data, duplicate document 처리를 점검한다.
- Evidence selection scoring과 candidate evidence card 생성 기준을 개선한다.

완료 기준:

- RAG 문서가 어디에서 왔고 왜 선택됐는지 설명할 수 있다.
- run 기반 축적과 사전 지식베이스 구축 전략이 분리된다.
- Evidence 기반 상품 생성 실패가 줄어든다.

## Phase 19: Evidence and Visual Evidence UX Redesign

목표:

- Evidence 탭이 “raw 근거 나열”이 아니라 “이 상품을 왜 믿을 수 있는지”를 설명하게 만든다.

작업:

- Evidence card UI를 사용자 관점으로 재설계한다.
- selection reason, 사용 가능한 claim, 확인 필요, 금지 claim을 구분해 보여준다.
- “추천 보강 호출”처럼 내부 구현 중심 문구를 제거하거나 사용자 의미가 있는 표현으로 바꾼다.
- 이미지 evidence는 상품 관련성이 높은 순서로 표시하고, 관련 없는 이미지는 제외 또는 낮은 우선순위로 둔다.
- Poster Studio 참조 이미지 후보와 Evidence 이미지 후보의 selection 기준을 통일한다.

완료 기준:

- 사용자가 Evidence 화면에서 상품별 근거의 강점과 공백을 이해할 수 있다.
- 상품과 맞지 않는 이미지가 주요 후보로 노출되는 빈도가 줄어든다.

## Phase 20: UI Copy and Product Surface Polish

목표:

- 사용자 화면에서 개발자용 변수명과 내부 구조를 제거하고, 전체 UI를 덜 경직된 운영툴 화면으로 다듬는다.

작업:

- source_id, raw field path, provider/tool/internal status 노출을 사용자 화면에서 제거한다.
- 사용자용 한국어 label 체계를 정리한다.
- Dashboard, Run Detail, Evidence + QA, Poster Studio의 정보 밀도와 spacing을 조정한다.
- card/table/badge/empty/error state 디자인을 통일한다.
- Developer 탭에는 필요한 debug 정보만 접어서 유지한다.

완료 기준:

- 일반 사용자 화면에서는 내부 구현 용어가 보이지 않는다.
- 화면이 운영툴답게 조용하지만, 지금보다 읽기 쉽고 덜 딱딱하다.

## Phase 21: Costs Dashboard

목표:

- LLM 호출, image generation, workflow run 비용과 latency를 운영자가 한 화면에서 추적한다.

작업:

- Costs 탭 placeholder를 실제 화면으로 전환
- `llm_calls`, poster asset cost/latency, workflow cost summary 집계 API 추가 또는 기존 API 확장
- run/model/provider/purpose별 비용 breakdown
- 날짜/기간, workflow run, provider filter
- 예상 비용과 실제 invoice 값이 아님을 명확히 표시
- high-cost run, retry-heavy run, failed external call 진단
- CSV/export 또는 report 링크 검토

완료 기준:

- AppShell Costs 탭에서 비용/latency summary와 상세 drilldown을 볼 수 있다.
- LLM과 image generation 비용 추정값의 출처와 한계를 구분한다.
- evaluation run과 일반 run 비용을 구분해서 볼 수 있다.
...
- 실제 TourAPI key와 Gemini/OpenAI key가 있으면 demo workflow와 Poster Studio smoke가 가능
- 실패 시 FastAPI log와 log file에서 원인을 확인할 수 있다.

## 다음 구현 시작점

Phase 10 Data Enrichment Workflow, Phase 10.1 AppShell Navbar and Global Navigation, Phase 10.2 Gemini Data Enrichment Agent 전환, Phase 10.5 UI and Operations Surface Cleanup, Phase 11 Evidence-based ProductAgent Actualization, Phase 11.5 Gemini Planner/Research Actualization and LLM Call Surface Cleanup, Phase 12.0 Retrieval Stability and Empty Result UX, Phase 12.1 Visual APIs Actual Connection, Phase 12.2 Route/Related/Demand Signals Actual Connection, Phase 12.3 Theme APIs Actual Connection, Phase 13 Evaluation and Quality Dashboard, Phase 14 Poster Studio는 구현 완료 상태입니다. 99번 문서에 있는 추가 KTO API를 실제로 호출하고 저장해 상품 생성에 활용하는 Phase 12 작업은 `12.1 Visual APIs`, `12.2 Route/Related/Demand Signals`, `12.3 Theme APIs`로 나눠 완료했습니다.

Codex에게 줄 다음 작업 범위:

```text
Phase 15: Quality Audit and Product Quality Baseline을 진행해줘.

범위:
- 최근 일반 run과 evaluation 결과를 샘플링해 QA/Marketing/Evidence/RAG/UI copy 실패 유형을 분류
- QA issue message, AI 수정 후 QA 재검수, Marketing copy 품질, Evidence selection, 이미지 relevance 문제를 구분
- source document 생성/저장/index/search 흐름을 코드와 DB 기준으로 확인
- 사용자 화면에서 제거해야 할 개발자용 문구와 내부 변수명을 목록화
- Phase 16~20 구현 우선순위와 테스트 기준을 문서화

주의:
- 이 Phase는 우선 진단/계획 문서화가 목적이다.
- Costs Dashboard는 Phase 21로, Deployment/Demo Hardening은 Phase 22로 미룬다.
```

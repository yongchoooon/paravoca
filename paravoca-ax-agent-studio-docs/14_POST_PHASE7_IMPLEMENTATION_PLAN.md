...
- prompt, raw metadata, latency, 추정 비용, failed error는 Developer 성격의 접힌 영역에서 확인할 수 있다.
...
```text
Phase 15: Costs Dashboard를 구현해줘.

범위:
- Costs 탭 placeholder를 실제 화면으로 전환
- llm_calls, workflow cost_summary, poster asset cost/latency 집계
- run/provider/model/purpose별 비용 breakdown
- evaluation run과 일반 run 구분
- 예상 비용과 실제 invoice 값이 아님을 명확히 표시
- high-cost/retry-heavy/failed external call 진단
```

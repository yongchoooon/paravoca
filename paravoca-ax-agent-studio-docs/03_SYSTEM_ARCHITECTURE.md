...
- `not_to_claim`과 선택한 QA issue는 prompt constraint로 포함합니다.
...
Data Agent는 `geo_scope` 기준으로 필요한 도구를 호출하고 source item을 저장합니다. 새 workflow의 primary 지역 필터는 legacy `areaCode`가 아니라 `lDongRegnCd`/`lDongSignguCd`입니다. Source document에는 `source_role`, `source_family`, `first_seen_run_id`, `last_seen_run_id`, `ingestion_method`를 저장해 runtime 근거, 기존 catalog 근거, 보강 결과, 수동 수집 근거를 구분합니다.

RAG 검색은 resolved location, target customer, preferred themes, retained narrow keyword, content type을 query/search context/filter에 반영합니다. 검색 결과가 0개이면 상위 지역이나 전국으로 자동 확장하지 않고, query/filter/result count/reason을 `retrieval_diagnostics`에 남깁니다.
  ...
  "issues": [
    {
      ...
      "issue_category": "evidence_risk",
      "message": "상세 설명에 문제 문구 '매일 야간 운영'이 있습니다. 운영시간이 근거 문서에 명확히 확인되지 않습니다.",
      "field_path": "marketing_assets[2].sales_copy.sections[1].body",
      "suggested_fix": "운영시간은 공식 확인 후 안내한다고 수정하세요."
    }
  ],
...
- 검색 조건을 만족하는 문서가 부족하면 자동 fallback하지 않고 부족 상태와 사용한 query/filter를 diagnostics에 남김
